...
foreach ($rResult as $aRow) {
    ...
    $rowName .= '<a href="#" onclick="contact(' . $aRow['userid'] . ',' . $aRow['id'] . ');return true;">' . _l('edit') . '</a>';
    ...
    // if (has_permission('customers', '', 'update') || is_customer_admin($aRow['userid'])) {
    //     if ($aRow['is_primary'] == 0 || ($aRow['is_primary'] == 1 && $total_client_contacts == 1)) {
    //         $rowName .= '  <a href="' . admin_url('clients/form_contact/' . $aRow['userid'] . '/' . $aRow['id']) . '" class="text-import _edit">' . _l('edit') . '</a>';
    //     }
    // }
